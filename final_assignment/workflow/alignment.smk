rule bwa_alignment:
    ...
    output:
        ...
        tmp_file = temp(config['alignment-dir'] + "{sample}_{type}_1.sai.sa"),
        tmp_file2 = temp(config['alignment-dir'] + "{sample}_{type}_2.sai.sa")
    message: "Aligning on {input.R1} and {input.R2} to produce {output.aligned_sam}"
    ...
    if (( $(echo "$avglength > $threshold" | bc -l) )); then
        ...
        if [[ $inputfile == *single* ]]; then
            ...
            bwa samse {params.genome} {output.tmp_file} {input.R1} > {output.aligned_sam}
        else
            bwa aln -t {threads} {params.genome} {input.R1} > {output.tmp_file} 
